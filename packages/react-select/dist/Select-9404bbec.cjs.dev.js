  ...
  function Select(_props) {
    ...
    _this.onTouchEnd = function (event) {
      ...
      if (_this.controlRef && !_this.controlRef.contains(event.target) && _this.menuListRef && !_this.menuListRef.contains(event.target)) {
        try {
          var targetArray = event.composedPath();
          var target = Array.isArray(targetArray) && targetArray.length > 0 ? targetArray[0] : null;

          if (!_this.controlRef.contains(target) && !_this.menuListRef.contains(target)) {
            _this.blurInput();
          }
        } catch (_unused) {
          _this.blurInput();
